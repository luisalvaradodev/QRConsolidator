...
import { consolidateAndSuggest } from './utils/consolidationLogic';
...
function App() {
  ...
  const handleFilesProcessed = (data: InventoryItem[]) => {
    const consolidated = consolidateAndSuggest(data);
      ...
      <header className="bg-white shadow-sm border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center space-x-3">
            ...
            <div><h1 className="text-2xl font-bold text-gray-900">Consolidador de Inventarios</h1><p className="text-gray-600">Análisis y consolidación de inventarios farmacéuticos</p></div>
          </div>
        </div>
      </header>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {consolidatedData.length > 0 && (
          <div className="flex flex-col lg:flex-row gap-6">
            <div className="lg:flex-shrink-0"><FilterPanel data={consolidatedData} filters={filters} onFilterChange={setFilters} /></div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  ...
                  <div className="text-sm text-gray-600">
                    {filteredData.length !== consolidatedData.length ? (<span>Mostrando {filteredData.length.toLocaleString()} de {consolidatedData.length.toLocaleString()} productos</span>) : (<span>{consolidatedData.length.toLocaleString()} productos consolidados</span>)}
                  </div>
                </div>
              </div>
              <DataTable data={filteredData} tableState={tableState} onTableStateChange={setTableState} />
            </div>
            <div className="lg:flex-shrink-0"><ExportButtons data={filteredData} suggestionDays={suggestionDays} /></div>
          </div>
        )}
      </div>
    </div>
