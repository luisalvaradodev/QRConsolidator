...
const ExportButtons: React.FC<ExportButtonsProps> = ({ data, suggestionDays }) => {
  ...
  if (data.length === 0) {
    return (
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center space-x-2 mb-4"><Download className="h-5 w-5 text-gray-400" /><h3 className="font-semibold text-gray-800">Exportar Datos</h3></div>
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      <div className="flex items-center space-x-2 mb-4"><Download className="h-5 w-5 text-gray-600" /><h3 className="font-semibold text-gray-800">Exportar Datos</h3></div>
      <p className="text-sm text-gray-600 mb-4">Exportar {data.length.toLocaleString()} productos filtrados</p>
      <div className="space-y-3">
        <button onClick={handleExportExcel} className="w-full flex items-center justify-center space-x-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-3 rounded-lg font-medium transition-colors">
          <FileSpreadsheet className="h-4 w-4" /><span>Exportar a Excel</span>
        </button>
      </div>
      <p className="text-xs text-gray-500 mt-3 text-center">El archivo incluirá los filtros y los días de sugerencia actuales.</p>
    </div>
  );
};
