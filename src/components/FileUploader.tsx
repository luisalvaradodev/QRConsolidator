...
const FileUploader: React.FC<FileUploaderProps> = ({ onFilesProcessed, isProcessing, setIsProcessing, classificationSettings }) => {
  ...
  // --- No hay cambios en la lógica, solo en las clases (className) ---
    ...
    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-blue-500/30 rounded-xl p-4">
      ...
      <div
        className={`relative border-2 border-dashed rounded-xl p-6 text-center transition-all duration-300 ${
          dragActive 
            ...
            : 'border-gray-300 dark:border-gray-600 hover:border-blue-400 dark:hover:border-blue-500/50 hover:bg-blue-50 dark:hover:bg-blue-500/5'
        }`}
      ...
      >
        ...
        <button
          ...
          className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          disabled={isProcessing}
        >
          Seleccionar Archivos
        </button>
        <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">Máximo 10 archivos</p>
      ...
      {isProcessing && (
        <div className="mt-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Procesando...</span>
      ...
      {uploadedFiles.length > 0 && (
        <div className="mt-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300">Archivos ({uploadedFiles.length})</h3>
            {!isProcessing && (
              <button onClick={clearAll} className="text-sm text-gray-500 hover:text-gray-800 dark:hover:text-gray-300 transition-colors">
                Limpiar
              </button>
            )}
          </div>
          ...
          <div className="space-y-2 max-h-32 overflow-y-auto">
            {uploadedFiles.map(({ file, status, error }) => (
              <div key={file.name} className="flex items-center justify-between p-2 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700">
                <div className="flex items-center space-x-2 min-w-0">
                  <File className="h-4 w-4 text-gray-500 dark:text-gray-400 flex-shrink-0" />
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-gray-800 dark:text-gray-300 truncate" title={file.name}>{file.name}</p>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  {status === 'error' && <span className="text-xs text-red-500 dark:text-red-400" title={error}>Error</span>}
                  
                  {!isProcessing && (
                    <button onClick={() => removeFile(file.name)} className="p-1 hover:bg-gray-200 dark:hover:bg-gray-700 rounded transition-colors">
